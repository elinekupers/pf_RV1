...
for ratio = 1:5
    ...
    a=load(fullfile(baseFolder, 'data', expName, 'classification/rgc', subFolder, 'stimTemplate', ...
        sprintf('classifySVM_rgcResponse_Cones2RGC%d_absorptionrate_%d_defaultnophaseshift_%s.mat',ratio,eccen,subFolder)));
    ...
    data.withoutTemplate = b.P_svm(ratio,:);
    ...
    title({sprintf('SVM Classifier performance for RGC responses WITH template'),'4.5 deg eccen, L-only, no eye movements, no stimulus phase shifts'})
    ...
    title({sprintf('SVM Classifier performance for RGC responses WITHOUT template'),'4.5 deg eccen, L-only, no eye movements, no stimulus phase shifts'})
