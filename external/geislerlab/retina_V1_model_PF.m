function out = retina_V1_model_PF(target, background, task, tx, ty, fx, fy, ppd, Parameters, Stacks, verbose)
...
for ii = 1:nrTargets
    ...
    background = cast(background,'double'); %convert to doubles
    target  = cast(allTargets{ii},'double'); 
    Stacks = allStacks{ii};
    ...
    for qq = 1:length(fx)
        ...
        T_pool_stim(:, :, qq,ii) = abs(SAMP_cut_T.*DT_fov); % save for 2AFC calculation
        ...
        if strcmp(task, 'detection')
            %Predicted contrast threshold
            C(qq) = P.dp_crit.*N_eff./T_pool;
            C_db(qq) = 20*log10(C(qq)); %in decibels
            
        elseif strcmp(task, '2AFC')
            backgroundResponse(qq, ii) = N_eff;
            targetResponse(qq, ii) = T_pool; 
        end
    end
